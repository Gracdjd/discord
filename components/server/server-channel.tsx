...
export const ServerChannel = ({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "group px-2 py-1 rounded-md flex items-center ",
        "gap-x-2 w-full hover:bg-zinc-700/10  dark:hover:bg-zinc-700/50 transition",
        "mb-1 ",
        params?.channelId === channel.id && "bg-zinc-700/20 dark:bg-zinc-700"
      )}
    >
      <Icon className="flex-shrink-0 !w-3 !h-3 text-zinc-500 dark:text-zinc-400" />
      <p
        className={cn(
        ...
        <div className="ml-auto flex items-center gap-x-2">
          <ActionTooltip label="Edit">
            <Edit
              ...
              className="hidden group-hover:block h-3 w-3 text-zinc-500
      ...
      {channel.name == "general" && (
        <Lock
          className="ml-auto h-3 w-3 text-zinc-500
         dark:text-zinc-400"
        />
      )}
    </button>
